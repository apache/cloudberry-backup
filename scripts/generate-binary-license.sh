...
are statically linked, so the compiled code of the third-party components
listed below is bundled inside the shipped binaries themselves.  None of
these components are present in the source release; this section applies
to the binary artifacts only.

The list covers what is bundled, not what is merely depended upon.
Libraries that are resolved from the host at run time rather than
packaged with the binaries -- the system C library above all -- are
therefore deliberately not listed here, and neither are dependencies used
only to build or test the project.
